...
This is **AstroRock**, a strict port of the AstroRock C++ game (1995–97 Win95 asteroids game):
...
- C source at `C:DevelopmentAstroRock` (local read-only reference) for reference
...
1. **Fixed-point overflow**: AstroRock uses 16.16 fixed-point (`CFixed`) everywhere in gameplay. Match the C++ overflow/truncation behavior exactly — use wrapping arithmetic where the C++ would wrap.
...
3. **Determinism**: demo playback and (future) lockstep net depend on bit-identical update order, RNG draws, and fixed-point math. Never reorder updates or swap in float math.
